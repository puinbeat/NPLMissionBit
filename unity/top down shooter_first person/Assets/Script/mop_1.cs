...
public class mop_1 : MonoBehaviour {
	...
	void Start () {
		timer = Time.time;
		transform.position = new Vector3 (Random.Range (player.transform.position.x - 6, player.transform.position.x + 6), 1, Random.Range(player.transform.position.x - 6, player.transform.position.x + 6));
	}
